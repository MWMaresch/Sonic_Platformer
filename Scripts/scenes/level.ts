module scenes {
    export class Level extends objects.Scene {
        ...
        protected createGridFromTileGroups(numGrid: number[][]) {//, tileGroup: Array<Array<objects.Tile>>) {

            this._tileGrid = new Array<Array<objects.Tile>>(numGrid[0].length * 16);


            var tileGroup = new Array<Array<objects.Tile>>(numGrid[0].length * 16);

            for (var x = 0; x < numGrid[0].length * 16; x++) {
                this._tileGrid[x] = new Array<objects.Tile>(numGrid.length * 16);
            }
